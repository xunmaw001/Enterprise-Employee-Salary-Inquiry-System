<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="安排编号" prop="anpaibianhao">
            <el-input v-model="ruleForm.anpaibianhao" 
                placeholder="安排编号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.anpaibianhao" label="安排编号" prop="anpaibianhao">
              <el-input v-model="ruleForm.anpaibianhao" 
                placeholder="安排编号" readonly></el-input>
          </el-form-item>
        </div>
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="安排详情" prop="anpaixiangqing">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.anpaixiangqing" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.anpaixiangqing" label="安排详情" prop="anpaixiangqing">
                    <span v-html="ruleForm.anpaixiangqing"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.anpaixiangqing = this.ruleForm.anpaixiangqing.replace(reg,'../../../ssm6x717/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `gongzuoanpai/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.gongzuoanpaiCrossAddOrUpdateFlag = false;
						 this.parent.search();
