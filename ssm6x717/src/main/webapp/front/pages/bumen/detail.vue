...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','bumen');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','bumen');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('bumen', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussbumen/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('bumen', this.detail);
				this.$utils.msg('审核成功');
